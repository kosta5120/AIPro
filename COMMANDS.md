...
#   --load-mode        full | page | structured (default: structured)
#   --chunk-strategy   fixed | sentence | recursive (default: recursive)
...
Builds one ablation index (chunk_size 600) and evaluates five configs:
dense k=3/5/8, BM25 k=5, and hybrid RRF k=5. Computes **Hit@k**, **MRR@k**,
and **Answer accuracy** (token F1), and writes results to `eval/results.md`
and `eval/answers_sample.jsonl`.
...
> Requires `reportlab` (not in `requirements.txt`): `pip install reportlab`
